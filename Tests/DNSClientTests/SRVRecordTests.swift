...
struct SRVRecordTests {
  ...
  func testInvalidSRVPriorities() {
    ...
    for priority in invalidPriorities {
      ...
      #expect(
        record.data?.isValidPriority == false, "SRVData priority \(priority) should be invalid")
    }
  }
  ...
  func testValidSRVServiceNames() {
    ...
    for serviceName in validServiceNames {
      #expect(
        SRVRecord.isValidServiceName(serviceName) == true,
        "Service name '\(serviceName)' should be valid")
      ...
      #expect(
        record.isValidServiceName == true,
        "Record with service name '\(serviceName)' should be valid")
    }
  }
  ...
  func testInvalidSRVServiceNames() {
    ...
    for serviceName in invalidServiceNames {
      #expect(
        SRVRecord.isValidServiceName(serviceName) == false,
        "Service name '\(serviceName)' should be invalid")
      ...
      #expect(
        record.isValidServiceName == false,
        "Record with service name '\(serviceName)' should be invalid")
    }
  }
  ...
  func testValidSRVTargetHostnames() {
    ...
    for hostname in validHostnames {
      ...
      #expect(
        record.data?.isValidTarget == true, "SRVData with target '\(hostname)' should be valid")
    }
  }
  ...
  func testInvalidSRVTargetHostnames() {
    ...
    for hostname in invalidHostnames {
      #expect(
        SRVRecord.isValidHostname(hostname) == false, "Hostname '\(hostname)' should be invalid")
  ...
  func testSRVServiceNameParsing() {
    ...
    for (fullName, expectedService, expectedProtocol, expectedDomain) in testCases {
      ...
      #expect(
        record.protocolName == expectedProtocol, "Protocol name should be '\(expectedProtocol)'")
