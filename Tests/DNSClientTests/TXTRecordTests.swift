...
struct TXTRecordTests {
    ...
    func testInvalidTXTContent() {
        ...
        for content in invalidContents {
            let record = TXTRecord(name: "example.com", content: content, ttl: .automatic)
            #expect(record.isValidContent == false, "Content should be invalid (length: \(content.count))")
            #expect(TXTRecord.isValidTextContent(content) == false, "Content should be invalid (length: \(content.count))")
        }
    }
    ...
    func testTXTContentQuoting() {
        ...
        for (input, expected) in testCases {
            ...
            #expect(quoted == expected, "Quoting '\(input)' should result in '\(expected)', got '\(quoted)'")
        }
    }
    ...
    func testTXTContentUnquoting() {
        ...
        for (input, expected) in testCases {
            ...
            #expect(unquoted == expected, "Unquoting '\(input)' should result in '\(expected)', got '\(unquoted)'")
        }
    }
    ...
    func testStructuredRecordDetection() {
        ...
        for (content, shouldBeStructured, expectedType) in structuredRecords {
            let record = TXTRecord(name: "example.com", content: content, ttl: .automatic)
            #expect(record.isStructuredRecord == shouldBeStructured, "Content '\(content)' structured detection should be \(shouldBeStructured)")
            #expect(record.structuredRecordType == expectedType, "Content '\(content)' should be detected as '\(expectedType)'")
        }
        ...
        for content in unstructuredRecords {
            let record = TXTRecord(name: "example.com", content: content, ttl: .automatic)
            #expect(record.isStructuredRecord == false, "Content '\(content)' should not be detected as structured")
            #expect(record.structuredRecordType == nil, "Content '\(content)' should not have a structured type")
        }
    }
    ...
    func testTXTRecordCommonUseCases() {
        ...
        let dmarcRecord = TXTRecord(
            name: "_dmarc.example.com",
            content: "v=DMARC1; p=quarantine; rua=mailto:dmarc-reports@example.com; ruf=mailto:dmarc-failures@example.com; fo=1",
            ttl: .seconds(3600)
        )
