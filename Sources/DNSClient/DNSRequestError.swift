...
public enum DNSRequestError: Error, Equatable {
  ...
  public static func == (lhs: DNSRequestError, rhs: DNSRequestError) -> Bool {
    ...
    case (
      .batchSizeExceeded(let lhsCount, let lhsMax), .batchSizeExceeded(let rhsCount, let rhsMax)
    ):
      return lhsCount == rhsCount && lhsMax == rhsMax
    case (.emptyBatch, .emptyBatch):
      return true
    case (
      .batchValidationFailed(let lhsIndex, let lhsError),
      .batchValidationFailed(let rhsIndex, let rhsError)
    ):
