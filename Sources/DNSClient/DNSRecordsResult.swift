...
public struct AnyDNSRecord: Sendable, Codable, Hashable {
    ...
    default:
      ...
      throw DecodingError.dataCorrupted(
        DecodingError.Context(
          ...
          debugDescription:
            "Unsupported DNS record type: \(type.rawValue). Supported types are: A, AAAA, CNAME, MX, TXT, SRV, CAA, NS, PTR"
        )
      )
    }
  }
  ...
  public static func == (lhs: AnyDNSRecord, rhs: AnyDNSRecord) -> Bool {
    return lhs._record.id == rhs._record.id && lhs._record.type == rhs._record.type
