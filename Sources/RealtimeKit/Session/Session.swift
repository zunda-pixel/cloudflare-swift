...
public struct Session: Codable, Hashable, Sendable, Identifiable {
  ...
  public init(from decoder: any Decoder) throws {
    ...
    self.maxConcurrentParticipants = try container.decode(
      Int.self,
      forKey: .maxConcurrentParticipants
    )
    ...
    self.groupCallMinutesConsumed = try container.decode(
      Double.self,
      forKey: .groupCallMinutesConsumed
    )
    self.webinarMinutesConsumed = try container.decode(Double.self, forKey: .webinarMinutesConsumed)
    self.audioRoomMinutesConsumed = try container.decode(
      Double.self,
      forKey: .audioRoomMinutesConsumed
    )
    self.liveStreamMinutesConsumed = try container.decode(
      Double.self,
      forKey: .liveStreamMinutesConsumed
    )
    self.chatMinutesConsumed = try container.decode(Double.self, forKey: .chatMinutesConsumed)
    self.recordingMinutesConsumed = try container.decode(
      Double.self,
      forKey: .recordingMinutesConsumed
    )
    self.transcriptionMinutesConsumed = try container.decode(
      Double.self,
      forKey: .transcriptionMinutesConsumed
    )
    ...
    self.startedAt = try Date(
      container.decode(String.self, forKey: .startedAt),
    ...
    self.endedAt = try container.decodeIfPresent(String.self, forKey: .endedAt).map {
      try Date($0, strategy: .iso8601WithFractionalSeconds)
    }
  ...
  public func encode(to encoder: any Encoder) throws {
    ...
    try container.encode(
      self.startedAt.formatted(.iso8601WithFractionalSeconds),
      forKey: .startedAt
    )
    try container.encode(
    ...
    try container.encodeIfPresent(
      self.endedAt.map { $0.formatted(.iso8601WithFractionalSeconds) },
      forKey: .endedAt
    )
