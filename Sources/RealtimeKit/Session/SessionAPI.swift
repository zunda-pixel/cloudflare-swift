...
extension Client {
  ...
  public func sessionParticipant(
    for sessionId: Session.ID,
    participantId: Session.User.ID
  ) async throws -> Session.User {
  ...
  public func sessionSummaryTranscript(
    for sessionId: Session.ID
  ) async throws -> SummaryTranscriptResponse {
    ...
    let response = try JSONDecoder().decode(
      SingleResponse<SummaryTranscriptResponse>.self, from: data)
    return response.data
  }

  public func generateSessionSummaryTranscript(
    for sessionId: Session.ID
  ) async throws -> GenerateSessionSummaryTranscriptResponse {
    let url = baseURL.appendingPathComponent("sessions/\(sessionId)/summary")
    let request = HTTPRequest(
      method: .post,
      url: url
    ...
    let (data, _) = try await execute(request)
    let response = try JSONDecoder().decode(
      SingleResponse<GenerateSessionSummaryTranscriptResponse>.self, from: data)
    return response.data
  }
